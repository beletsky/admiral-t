...
/**
 * –асстановка "м€гких" переносов в словах.
 * Ѕраузеры, которые показывают их: IE 6.0.x, Opera 7.54u2, Safari 3.1.1, Firefox 3.0.0
 * ѕоддерживаетс€ текст дл€ русского (UTF-8) и английского €зыков (ANSI).
 *
 ...
 * @param   string   $is_html  если TRUE, то html таги, комментарии и сущности не обрабатываютс€
 * @return  string
 *
...
function hyphen_words($s, $is_html = false)
{
    ...
    #регул€рное выражение дл€ атрибутов тагов
    #корректно обрабатывает гр€зный и битый HTML в однобайтовой или UTF-8 кодировке!
    $re_attrs_fast_safe =  '(?> (?>[\x20\r\n\t]+|\xc2\xa0)+  #пробельные символы (д.б. об€зательно)
                                (?>
                                  #правильные атрибуты
                                                                 [^>"\']+
                                  | (?<=[\=\x20\r\n\t]|\xc2\xa0) "[^"]*"
    ...
    $regexp = '/(?: #встроенный PHP, Perl, ASP код
                  ...
                  | <\!\[CDATA\[ .*? \]\]>
                    ...
                    #условное выполнение кода дл€ IE типа "<!--[if lt IE 7]>...<![endif]-->"
                  | <\! (?>--)?
                        \[
                        (?> [^\]"\']+ | "[^"]*" | \'[^\']*\' )*
                        \]
...
function _hyphen_words(array &$m)
{
    ...
    #буква (letter)
    $l = '(?: \xd0[\x90-\xbf\x81]|\xd1[\x80-\x8f\x91]  #ј-€ (все)
            | [a-zA-Z]
          )';
    ...
    $l_en = '[a-zA-Z]';
    #буква (letter)
    $l_ru = '(?: \xd0[\x90-\xbf\x81]|\xd1[\x80-\x8f\x91]  #ј-€ (все)
             )';

    #гласна€ (vowel)
    $v = '(?: \xd0[\xb0\xb5\xb8\xbe]|\xd1[\x83\x8b\x8d\x8e\x8f\x91]  #аеиоуыэю€Є (гласные)
            | \xd0[\x90\x95\x98\x9e\xa3\xab\xad\xae\xaf\x81]         #ј≈»ќ”џЁёя® (гласные)
            | (?i:[aeiouy])
          )';

    #согласна€ (consonant)
    $c = '(?: \xd0[\xb1-\xb4\xb6\xb7\xba-\xbd\xbf]|\xd1[\x80\x81\x82\x84-\x89]  #бвгджзклмнпрстфхцчшщ (согласные)
            | \xd0[\x91-\x94\x96\x97\x9a-\x9d\x9f-\xa2\xa4-\xa9]                #Ѕ¬√ƒ∆« ЋћЌѕ–—“‘’÷„Ўў (согласные)
            | (?i:sh|ch|qu|[bcdfghjklmnpqrstvwxz])
          )';

    #специальные
    $x = '(?:\xd0[\x99\xaa\xac\xb9]|\xd1[\x8a\x8c])';   #…Џ№йъь (специальные)

    if (0)
    {
        #алгоритм ѕ.’ристова в модификации ƒымченко и ¬арсанофьева
        $rules = array(
            # $1       $2
            "/($x)     ($l$l)/sxS",
            "/($v)     ($v$l)/sxS",
