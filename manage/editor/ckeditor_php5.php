...
class CKEditor
{
     ...
     * @endcode
     */
    public function editor($name, $value = "", $config = array(), $events = array(), $js_var = "")
    {
        $attr = "";
        ...
        if (!empty($_config))
            $js .= "var editor".$js_var."=CKEDITOR.replace('".$name."', ".$this->jsEncode($_config).");";
        else
            $js .= "var editor".$js_var."=CKEDITOR.replace('".$name."');";
